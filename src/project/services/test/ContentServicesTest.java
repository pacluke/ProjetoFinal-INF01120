...
public class ContentServicesTest {
	...
	public void addQuestionTest() throws Exception{
		...
		/*cs[0].addQuestion(title[0], text, user[0], listaTag); //can't test this because it returns a "PERMISSION DENIED" to 
									//adding a question by an anonymous user and the test stops running
		
		List<Object> questionsss = database.search(null);

		for(int i = 0; i < questionsss.size(); i++){
			System.out.println("oi");
			if(((Question)questionsss.get(i)).getTitle() == q[1].getTitle()){
				System.out.println("chegou" + ((Question)questionsss.get(i)).getTitle());
				feedback[0] = true;
			}
		}
		
		assertFalse(feedback[0]);
		*/
